...
Require-Pattern 'prompts/03-marca-e-layout/13-criar-layout-inicial.md' '(?s)(?=.*Ver visual)(?=.*links Markdown clic.veis)(?=.*Abrir aplica..o)(?=.*Ver live preview)(?=.*Ver\s+screenshots)(?=.*Ver v.deo)(?=.*n.o selecion.vel.*nunca a recomendes)' 'As tabelas do prompt 13 nao tornam cada sugestao visualmente inspecionavel e clicavel.'
...
Require-Pattern 'PROMPT_EVALUATION.md' '(?s)EVAL-05(?=.*nove linhas.*Ver visual)(?=.*link Markdown clic.vel)(?=.*live preview exato)(?=.*store listing com screenshots)(?=.*link quebrado)(?=.*n.o selecion.vel.*nunca . recomendada)' 'EVAL-05 nao verifica previews clicaveis e falhas de links por direcao.'
...
Require-Pattern 'pilot/cases/EVAL-05.md' '(?s)(?=.*coluna `Ver visual`)(?=.*nove linhas.*link Markdown clic.vel)(?=.*live preview exato)(?=.*store listing com\s+screenshots)(?=.*preview inicialmente quebrado)(?=.*n.o selecion.vel.*n.o a recomenda)' 'O caso EVAL-05 nao exercita links visuais validos para todas as sugestoes.'
