...
Require-Pattern 'AGENTS.md' '(?s)(?=.*sempre que o programador autorizar `commit e sync`)(?=.*cachebuster oficial)(?=.*antes de criar o commit)(?=.*integra o remoto sem force push)(?=.*codex plugin add advance-app@promptsadvance)(?=.*codex plugin list)(?=.*installed, enabled)(?=.*working tree limpa)(?=.*tarefa nova)(?=.*parcial)(?=.*segundo commit vazio)' 'O projeto nao obriga commit/sync a atualizar e verificar a tool Advance sem recursao.'
Require-Pattern 'README.md' '(?s)`commit e sync` atualiza tamb.m a\s+tool Advance.*cachebuster.*inclu.do no pr.prio commit.*codex plugin add advance-app@promptsadvance.*codex plugin list.*task nova' 'O README nao documenta a atualizacao automatica da tool depois do sync.'
...
Require-Pattern 'PROMPT_EVALUATION.md' '(?s)EVAL-02-SYNC.*remote Git local descart.vel.*cachebuster.*antes do commit.*codex plugin add advance-app@promptsadvance.*installed, enabled.*working tree final fica limpa.*parcial.*segundo commit vazio' 'A avaliacao nao cobre commit, sync e reinstalacao da tool como um unico workflow.'
...
Require-Pattern 'prompts/03-marca-e-layout/13-criar-layout-inicial.md' '(?s)(?=.*Reset visual obrigat.rio antes da proposta)(?=.*Remove primeiro)(?=.*layouts/shells)(?=.*CSS/SCSS)(?=.*componentes UI do BitPlatform)(?=.*n.o reutilizes componentes visuais pr.prios)(?=.*INITIAL_LAYOUT_RESET\.md)(?=.*pesquisas reproduz.veis)(?=.*resultado `parcial`)' 'O prompt 13 nao elimina e comprova a remocao da camada visual e dos componentes BitPlatform antes de criar a proposta do zero.'
Require-Pattern 'prompts/03-marca-e-layout/13-criar-layout-inicial.md' '(?s)(?=.*Preserva rotas, contratos)(?=.*autentica.*autoriza.*regras de neg.*cio)(?=.*infraestrutura BitPlatform.*visual)(?=.*Reimplementa do zero)(?=.*apagues.*gerado)' 'O reset visual do prompt 13 pode destruir comportamento ou infraestrutura nao visual.'
Require-Pattern 'prompts/03-marca-e-layout/13-criar-layout-inicial.md' '(?s)(?=.*exce.*regra gen.*preservar)(?=.*Todas as rotas existentes voltam a compilar)(?=.*primitivas novas e semanticamente m.*nimas)(?=.*restantes\s+rotas podem ficar visualmente)(?=.*podem ficar quebradas)(?=.*contrato OpenAPI real)(?=.*layout, tema ou CSS anterior de OpenAPI/Swagger/Scalar)' 'O reset visual do prompt 13 nao resolve o conflito de preservacao nem mantem as rotas e a API utilizaveis durante a substituicao.'
...
Require-Pattern 'PROMPT_EVALUATION.md' '(?s)EVAL-05(?=.*INITIAL_LAYOUT_RESET\.md)(?=.*componentes UI do BitPlatform)(?=.*rotas, contratos, permiss.*es, neg.*cio)' 'EVAL-05 nao verifica o reset visual sem regressao funcional.'
Require-Pattern 'pilot/cases/EVAL-05.md' '(?s)(?=.*prompts 13, 16 e 17)(?=.*INITIAL_LAYOUT_RESET\.md)(?=.*componentes UI\s+do BitPlatform)(?=.*templates pagos premium)(?=.*INITIAL_LAYOUT_RESEARCH\.md)(?=.*INITIAL_LAYOUT_SPEC\.md)(?=.*INITIAL_LAYOUT_CRITIQUE\.md)(?=.*tarefa separada)(?=.*findings cr.*ticos/altos)' 'O caso EVAL-05 nao reproduz a exigencia visual ponta a ponta.'
...
Require-Pattern 'software-lifecycle.ps1' '(?s)Get-PromptIdentityMap.*explicit migration because prompt.*identity mapping changed' 'O upgrade nao bloqueia uma renumeracao que reutiliza IDs para outros prompts.'
