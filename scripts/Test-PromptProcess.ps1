...
Require-Pattern 'pilot/cases/EVAL-05.md' '(?s)(?=.*prompts 13, 16 e 17)(?=.*`pr.ximo` n.o . uma escolha)(?=.*INITIAL_LAYOUT_DECISION\.md)(?=.*INITIAL_LAYOUT_RESET\.md)(?=.*INITIAL_LAYOUT_AUDIT\.md)(?=.*componentes UI\s+do\s+BitPlatform)(?=.*templates pagos premium)(?=.*INITIAL_LAYOUT_RESEARCH\.md)(?=.*INITIAL_LAYOUT_SPEC\.md)(?=.*INITIAL_LAYOUT_CRITIQUE\.md)(?=.*tarefa separada)(?=.*findings cr.*ticos/altos)' 'O caso EVAL-05 nao reproduz decisao, pesquisa e qualidade visual ponta a ponta.'
