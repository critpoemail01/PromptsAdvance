...
Audita e corrige a experiência PWA de `Client.Web` para `[BROWSERS_E_DISPOSITIVOS]`, focando manifest, instalação, modo standalone, comportamento offline, dados/operações pendentes e atualização vista pelo utilizador. A política HTTP/CDN de publicação pertence ao prompt 57 e a sua validação online ao prompt 68.
...
1. Lê service worker, manifest, estratégia do prompt 57 e resultados online do prompt 68 quando já existirem, configuração de publicação e documentação de force update do boilerplate.
...
Apresenta matriz de instalação/offline/sync/atualização, ficheiros, browsers/dispositivos, testes/resultados, evidência de atualização, dados que nunca ficam offline e questões encaminhadas para os prompts 57/68.
