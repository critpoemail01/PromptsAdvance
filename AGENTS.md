...
- Neste repositório canónico `AdvanceAppFlow`, sempre que alterares qualquer
  ...
  resposta final, executa o cachebuster oficial sobre `plugins/advance-app`,
  valida o plugin e as skills, reinstala com
  `codex plugin add advance-app@promptsadvance` e confirma em `codex plugin list`
  que está `installed, enabled` na versão exata do manifesto. Isto não autoriza
  commit nem push quando não foram pedidos; as alterações e o novo cachebuster
  podem permanecer locais. Quando o programador autorizar `commit e sync`, gera
  um único cachebuster antes do commit, inclui-o nesse commit, integra o remoto
  sem force push, faz push e só depois reinstala essa mesma versão. A atualização
  do cachebuster/reinstalação que fecha o workflow não volta a disparar esta
  regra nem cria um segundo bump ou commit vazio. Informa sempre que é necessária
  uma tarefa nova para carregar as skills. Se a validação, reinstalação ou
  confirmação falhar, reporta `parcial` e não declares a tool atualizada. Depois
  de reinstalar, executa `scripts/Update-AdvanceLocalProjects.ps1` para propagar
  o catálogo às instâncias lifecycle conhecidas sob `SoftwareProcesses`: usa
  `-AcceptCandidateCatalog` apenas quando esta autorização duradoura do
  programador se aplicar, preserva estado e evidências através do `upgrade`
  oficial e valida cada instância. Não trata clones, backups ou diretórios sem
  `LIFECYCLE_STATE.json` como instâncias; não altera lifecycles concluídos, com
  tentativa ativa, inválidos ou com marcadores de recuperação/atualização
  concorrente, e informa o motivo por projeto. A instalação global do plugin já
  serve os restantes projetos locais que não possuem uma cópia lifecycle. Depois
  de `commit e sync`, termina também com a working tree limpa.
