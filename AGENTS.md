...
- Neste repositório canónico `PromptsAdvance`, sempre que o programador autorizar `commit e sync`, inclui a atualização da tool Advance no mesmo workflow: antes de criar o commit executa o cachebuster oficial sobre `plugins/advance-app` para que a nova versão fique versionada; valida e inclui essa alteração no commit; integra o remoto sem force push e faz push; imediatamente depois do push reinstala com `codex plugin add advance-app@promptsadvance` e confirma em `codex plugin list` que está `installed, enabled` na versão exata do manifesto. Termina com a working tree limpa e informa que uma tarefa nova é necessária para carregar as skills. Se Git sincronizar mas a reinstalação/verificação falhar, reporta `parcial` e não declares a tool atualizada. Não cries um segundo commit vazio apenas por causa da reinstalação.
